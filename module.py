...
class InstanceNormalization(Layer):
    def __init__(self, epsilon=1e-6, activation=None, **kwargs):
    ...
    def build(self, input_shape):
        self.gamma = self.add_weight(
            shape=(input_shape[-1],),
            initializer=RandomNormal(1.0, 0.02),
            trainable=True,
            name='gamma'
        )
...
def instance_norm_layer(
    ...
    layer = InstanceNormalization(epsilon=epsilon, activation=activation_fn, name=name)
    return layer(inputs)
